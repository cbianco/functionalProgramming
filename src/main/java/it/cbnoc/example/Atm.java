...
import it.cbnoc.state.Condition;
import it.cbnoc.state.StateMachine;
import it.cbnoc.state.Transition;
...
public class Atm {
	public static StateMachine<Input, Outcome> createMachine() {
		...
		List<Tuple2<Condition<Input, Outcome>,
			Transition<Input, Outcome>>> transitions = List.list(
			new Tuple2<>(predicate1, transition1),
			new Tuple2<>(predicate2, transition2),
			new Tuple2<>(predicate3, transition3));
