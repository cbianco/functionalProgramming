...
public class Manager extends AbstractActor<Integer> {
	...
	private final
		Function<Manager, Function<Behavior, Effect<Integer>>> managerFunction;

	public Manager(
		String id, List<Integer> list, Actor<Result<List<Integer>>> client,
		int workers) {

		super(id, Type.SERIAL);
		this.client = client;
		this.workers = workers;
		Tuple<List<Integer>, List<Integer>> listListTuple =
			list.splitAt(this.workers);

		this.initial = listListTuple._1.zipWithPosition();
		this.workList = listListTuple._2;
		...
		managerFunction = manager -> behavior -> i -> {
			...
			if(result.length() == list.length()) {
				...
				manager.getContext()
					.become(new Behavior(behavior.workList.tailOption().getOrElse(List.list()), result));
			}
		};
	}
	...
	class Behavior implements MessageProcessor<Integer> {
		...
		Behavior(List<Integer> workList, List<Integer> resultList) {
		...
		public void process(Integer i, Result<Actor<Integer>> sender) {
			managerFunction.apply(
				Manager.this).apply(Behavior.this).apply(i);

			sender.forEach(a -> workList.headOption().forEachOrFail(x ->
				a.tell(x, self())).forEach(y -> a.shutdown()));
